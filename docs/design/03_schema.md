...
| `id`                 | text PK          |                                   |
| `title`              | text NOT NULL    |                                   |
| `notes`              | text NULL        | markdown project doc              |
| `status`             | text NOT NULL    | `idea` \| `in_progress` \| `done` |
| `priority`           | integer NULL     | 1–5, optional                     |
| `target_when`        | text NULL        | rough timing hint                 |
| `budget_cents`       | integer NULL     | optional project budget           |
...
Rollups (`estimated_cost_cents`, `acquired_cost_cents`, `remaining_cost_cents`) are computed at query time from component rows.
...
Migrations run automatically on server startup. Use `pnpm db:migrate` to apply manually if needed. Seed data optional for dev only — not committed.
