...
import { updateHomeSpaceNotes } from "@/lib/actions/home";
...
export function HomeSpaceNotesEditor({
  spaceId,
  initialNotes,
  users = [],
}: {
  ...
  const saveNotes = useCallback(
    async (value: string) => updateHomeSpaceNotes(spaceId, value),
    [spaceId],
  );
  ...
  async function handleBlur() {
    await flush();
    await updateHomeSpaceNotes(spaceId, notes);
  }
    ...
    <section className="rounded-lg border border-border bg-surface p-4 shadow-card">
      ...
      {mode === "edit" ? (
        <MentionTextarea
          ...
          placeholder="Notes about this space — finishes, history, quirks…"
        />
      ) : (
