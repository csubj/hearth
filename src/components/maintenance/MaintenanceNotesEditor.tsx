...
import { updateMaintenanceNotes } from "@/lib/actions/maintenance";
...
export function MaintenanceNotesEditor({
  logId,
  initialNotes,
  users = [],
}: {
  ...
  const saveNotes = useCallback(
    async (value: string) => updateMaintenanceNotes(logId, value, { reconcileMentions: false }),
    [logId],
  );
  ...
  async function handleBlur() {
    await flush();
    await updateMaintenanceNotes(logId, notes, { reconcileMentions: true });
  }
    ...
    <section className="rounded-lg border border-border bg-surface p-4 shadow-card">
      ...
      {mode === "edit" ? (
        <MentionTextarea
          ...
          placeholder="Work performed, warranty info, follow-up notes…"
        />
      ) : (
