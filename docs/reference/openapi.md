...
The spec documents every `/api/v1/*` resource, request/response schemas, error shapes, and the bearer-token security scheme. It is the authoritative reference for field names, types, and pagination parameters.

## How the spec is generated

hearth does not maintain a hand-written OpenAPI file. Instead:

1. **Zod schemas** in `src/lib/api/schemas/` define request bodies, query params, and response shapes
2. **`@asteasolutions/zod-to-openapi`** registers those schemas and route definitions into an OpenAPI registry
3. **`GET /api/openapi.json`** emits the registry as JSON at runtime — always in sync with the handlers
...
`/api/docs` renders the spec in a browser UI (Scalar or Redoc — see [Tech Choices](../design/01_tech.md)). Use it to:
