            ...
            {entries.map((entry) => (
              <tr key={entry.id} className="border-b border-border last:border-b-0">
                ...
                <td className="px-4 py-3 font-medium text-text">
                  {formatValue(entry.value, tracker.unit)}
                </td>
                <td className="px-4 py-3 text-text">
                  <div className="space-y-2">
                    <span>{entry.note ?? "—"}</span>
                    <Suspense fallback={<p className="text-xs text-text-muted">Loading photos…</p>}>
                      <Attachments entityType="tracker_entry" entityId={entry.id} />
                    </Suspense>
                  </div>
                </td>
              </tr>
        ...
        {entries.map((entry) => (
          <li key={entry.id} className="rounded-lg border border-border bg-surface p-4 shadow-card">
            <div className="flex items-start justify-between gap-3">
              <p className="font-medium text-text">{formatValue(entry.value, tracker.unit)}</p>
